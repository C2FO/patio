var pg = require("pg.js"),
    PgTypes = require("pg.js/lib/types"),
    comb = require("comb"),
            ...
            return asyncArray(this.execute(sql).chain(function (rows, fields) {
                ...
                if (rows && rows.length) {
                    ...
                    if (fields && fields.length) {
                        l = fields.length;
                        while (++i < l) {
                            colOutputIdentifier = oi(col = fields[i].name);
                        ...
                        l = fields.length;
                        while (++i < l) {
                            colOutputIdentifier = oi(col = fields[i]);
                            cols[i] = [colOutputIdentifier, colCallback, col];
                        }
                    }
                }
                var ret = self.__processRows(rows, cols);
                return ret;
            }));
        },
            ...
            columns: function () {
                ...
                if (this.__columns) {
                    ...
                    ret = this.db.schema(this.firstSourceTable).chain(function (schema) {
                        return (self.__columns = schema ? Object.keys(schema) : []);
                    });
                }
                return ret.promise();
            },
    ...
    instance: {
        ...
        createConnection: function (opts) {
            ...
            conn.on("error", function (err) {
                self.logWarn("Connection from " + self.uri + " removing from pool and reconnecting");
            ...
            return (ret = new Connection(conn));
        },
        ...
        execute: function (sql, opts, conn) {
            ...
            if (conn) {
                ...
                return this._getConnection().chain(function (conn) {
                    return self.__execute(conn, sql, opts);
                });
            }
        },
