    ...
    instance:{
        ...
        fullTextSearch:function (cols, terms, opts) {
            ...
            return this.filter("to_tsvector(?, ?) @@ to_tsquery(?, ?)", lang, this.__fullTextStringJoin(toArray(cols).map(function (c) {
                return sql.stringToIdentifier(c);
            })), lang, terms);
        },
...
//        },
        ...
        _selectWithSql:function () {
            ...
            if (!comb.isEmpty(optsWith) && optsWith.some(function (w) {
                return w.recursive;
            })) {
        ...
        __fullTextStringJoin:function (cols) {
            ...
            cols.pop();
            return sql.StringExpression.fromArgs(['||'].concat(cols));
        },
        ...
        _literalTimestamp:function (v) {
            return this.literal(sql.literal("TIMESTAMP " + this._super(arguments) + ""));
        },

        _literalBuffer:function (b) {
            return this.literal(sql.literal("decode('" + b.toString("hex") + "', 'hex')"));
        },
    ...
    "static":{
        ...
        BOOL_TRUE:'true',
        COMMA_SEPARATOR:', ',
        DELETE_CLAUSE_METHODS:Dataset.clauseMethods("delete", 'with from using where returning'),
        EXCLUSIVE:'EXCLUSIVE',
        EXPLAIN:'EXPLAIN ',
        ...
        TIMESTAMP_FORMAT:"yyyy-MM-dd hh:mm:ss.SSSS",
        QUERY_PLAN:'QUERY PLAN',
        ROW_EXCLUSIVE:'ROW EXCLUSIVE',
        ROW_SHARE:'ROW SHARE',
        SELECT_CLAUSE_METHODS:Dataset.clauseMethods("select", 'with distinct columns from join where group having compounds order limit lock'),
        SHARE:'SHARE',
    ...
    instance:{
        ...
        execute:function (sql, opts, cb, conn) {
            ...
            connP.then(hitch(this, function (conn) {
                this.__execute(conn, sql, opts, cb).then(ret);
            }), hitch(ret, "errback"));
            return ret;
        },
        ...
        indexes:function (table, opts) {
            ...
            when(this.serverVersion()).then(function (version) {
                ...
                if (schema) {
                    ds = ds.join("pg_namespace___nsp", {oid:sql.tab__relnamespace, nspname:schema.toString()});
                }
        ...
        locks:function () {
            return this.dataset.from("pg_class").join("pg_locks", {relation:sql.relfilenode}).select("pg_class__relname", sql.pg_locks.all());
        },
...
//            " AND pg_class.relname = '#{table}'
            var ds = this.from(table)
                ...
                .where([
                [sql.identifier("pg_class__oid"), sql.identifier("pg_attribute__attrelid")],
                [sql.identifier("pg_class__relnamespace"), sql.identifier("pg_namespace__oid")],
                [sql.identifier("pg_class__oid"), sql.identifier("pg_index__indrelid")],
                [sql.identifier("pg_index__indkey").sqlSubscript(0), sql.identifier("pg_attribute__attnum")],
                [sql.identifier("indisprimary"), true],
                [sql.identifier("pg_class__relname"), table.toString()]
            ]);
            if (schema) {
        ...
        __indexDefinitionSql:function (tableName, index) {
            tableName = sql.stringToIdentifier(tableName);
            var cols = index.columns.map(function (col) {
                    return sql.stringToIdentifier(col);
                }),
