...
var Model = comb.define([QueryPlugin, AssociationPlugin, comb.plugins.Middleware], {
    instance:{
        ...
        _checkIfColumnIsPrimaryKey : function(name){
            var col = this.schema[name];
            if (col.primaryKey && !this.__ignore && this._static.isRestrictedPrimaryKey) {
        ...
        _setColumnValue : function(name, val){
            ...
            this._checkIfColumnIsPrimaryKey(name);
...
exports.create = function (name, modelOptions) {
    ...
    var DEFAULT_PROTO = {instance:{_hooks:["save", "update", "remove", "load"], __hooks:{pre:{}, post:{}}}};
    ...
    var model, ret = new comb.Promise();
    ...
    if ((model = MODELS.get(db)[checkAndTransformName(tableName.toString())]) != null) {
        ...
        model = comb.define([Model].concat(modelOptions.plugins || []), modelOptions);
        model._setTableName(tableName);
        model._setDb(db);
