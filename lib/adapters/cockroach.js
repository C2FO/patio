...
PgTypes.setTypeParser(20, 'text', function (val) {
    ...
    if (!getPatio().parseInt8) {
        console.log('skip parsing and return as is...')
        return val;
    }
        ...
        serverVersion: function () {
            if (!this.__serverVersion) {
              ...
              this.__serverVersion = this.get(identifier("version").sqlFunction).chain(function (version) {
                  // CockroachDB CCL v19.2.2 (x86_64-unknown-linux-gnu, built 2019/12/11 01:33:43, go1.12.12)
                  // var m = version.match(/CockroachDB (\d+)\.(\d+)(?:(?:rc\d+)|\.(\d+))?/);
                  // Just hacked up to get it ready
                  var version = version.split(" ")[2]
                    .replace("v", "");
                  console.log(version);
