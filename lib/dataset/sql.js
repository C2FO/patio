    ...
    instance:{
        ...
        _selectWithSql:function (sql) {
            ...
            if (wit && wit.length) {
                ...
                var base = sql.join("");
                sql.length = 0;
                sql.push((format("%s %s %s", this._selectWithSqlBase(), wit.map(
                    function (w) {
                        var r = [];
                        r.push(this.quoteIdentifier(w.name));
                        w.args && (r.push(format("(%s)", this.__argumentList(w.args))));
                        r.push(" AS ", this._literalDataset(w.dataset));
                        return r.join("");

                    }, this).join(this._static.COMMA_SEPARATOR), base)));
            }
        },
