...
const toArray = comb.array.toArray;
const ModelError = require("./errors").ModelError;
const plugins = require("./plugins");
const isUndefinedOrNull = comb.isUndefinedOrNull;
const AssociationPlugin = plugins.AssociationPlugin;
const QueryPlugin = plugins.QueryPlugin;
const Promise = comb.Promise;
const PromiseList = comb.PromiseList;
const HashTable = comb.collections.HashTable;
const hitch = comb.hitch;
const Middleware = comb.plugins.Middleware;
...
const MODELS = new HashTable();

const applyColumnTransformMethod = function (val, meth) {
    return !isUndefinedOrNull(meth) ? isFunction(val[meth]) ? val[meth] : isFunction(comb[meth]) ? comb[meth](val) : val : val;
};

const Model = define([QueryPlugin, Middleware], {
    instance: {
        /**
...
var allModels = [];
/**@ignore*/
export function create (name, supers, modelOptions) {
    if (!patio) {
        (patio = require("./index"));
...
export function getModel (name, db) {
    var ret = null;
    if (isDefined(name)) {
