...
suite.addBatch({
    ...
    "graph should " : {
        ...
        "work on from_self datasets" : function() {
        ...
        "throw an error if a symbol, dataset, or object with dataset is not used" : function() {
            assert.throws(function() {
                ds1.graph({}, {x : "id"})
            });
        },

        "accept a :table_alias option"  : function(){
          ...
          "accept a :implicit_qualifier option"  : function(){
          ...
          "not select any columns from the graphed table if :select option is false"  : function(){
            ds = ds1.graph("lines", {x : "id"}, {select : false}).then(function(d){d.graph("graphs", {id : "graphId"}).then(function(ds){
          ...
          "use the given columns if :select option is used"  : function(){
            ds = ds1.graph("lines", {x : "id"}, {select : ["x", "graphId"]}).then(function(d){d.graph("graphs", {id : "graphId"}).then(function(ds){
          ...
          "pass all join_conditions to join_table"  : function(){
            ds = ds1.graph(ds2, [["x", "id"], ["y", "id"]]).then(function(ds){
...
suite.run({reporter : vows.reporter.spec});