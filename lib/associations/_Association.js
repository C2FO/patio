var comb = require("comb"), Promise = comb.Promise, PromiseList = comb.PromiseList, hitch = comb.hitch;
    ...
    instance:{
        ...
        _getAssociationKey:function () {
            ...
            if (!comb.isUndefinedOrNull((key = options.key))) {
                ...
                ret = [
                    comb.array.toArray(lk),
                    comb.array.toArray(rk)
                ];
            } else {
        ...
        _setDatasetOptions:function (ds) {
            ...
            if (!comb.isUndefined((select = this.select))) {
                ds = ds.select.apply(ds, comb.array.toArray(select));
            }
            ...
            if (!comb.isUndefined((distinct = options.distinct))) {
                ds.limit.apply(ds, comb.array.toArray(distinct));
            }
            if (!comb.isUndefined((order = options.orderBy)) || !comb.isUndefined((order = options.order))) {
                ds.order.apply(ds, comb.array.toArray(order));
            }
            if (!comb.isUndefined((limit = options.limit))) {
                ds.limit.apply(ds, comb.array.toArray(limit));
            }
            return ds;
        ...
        _toModel:function (val, fromDb) {
            if (!comb.isInstanceOf(val, this.model)) {
        ...
        getters:{
            ...
            //Returns our model
            model:function () {
                return this.patio.getModel(this._model);
            },

            associatedModelKey:function () {
                var ret = "";
                if (this.isOwner) {
                    ret = this.parent.tableName + "Id";
                } else {
