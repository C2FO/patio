...
const array = comb.array;
const flatten = array.flatten;
const ExpressionError = require("./errors").ExpressionError;
const methodMissing = require("./proxy").methodMissing;
const createFunctionWrapper = comb.createFunctionWrapper;
const isUndefined = comb.isUndefined;
const isUndefinedOrNull = comb.isUndefinedOrNull;
const isNull = comb.isNull;
const isInstanceOf = comb.isInstanceOf;
const argsToArray = comb.argsToArray;
const isDate = comb.isDate;
const isHash = comb.isHash;
const merge = comb.merge;
const isArray = comb.isArray;
const toArray = array.toArray;
const format = comb.string.format;
const isBoolean = comb.isBoolean;
const isNumber = comb.isNumber;
const isObject = comb.isObject;
const isString = comb.isString;
const define = comb.define;
const isRegExp = comb.isRegExp;

let Dataset, patio, sql, Expression, AliasedExpression, CaseExpression, Cast, ColumnAll, BooleanExpression,
...
class VirtualRow extends Function {
    constructor(name) {
        ...
        return new Proxy(function(){}, {
            apply(target, thisArg, argumentsList) {
                return argumentsList.length ? SQLFunction.fromArgs([name].concat(argumentsList)): ident;
            },
            construct(target, args) {
                return SQLFunction.fromArgs(args);
            },
            enumerate(target) {
                return Reflect.enumerate(ident);
            },
            get(object, property) {
                if (Reflect.has(ident, property)) {
                    return Reflect.get(ident, property);
                } else {
                    return (...args) => SQLFunction.fromArgs([property, name].concat(args));
                }
            },
            getOwnPropertyDescriptor(target, property) {
                return Reflect.getOwnPropertyDescriptor(ident, property);
            },
            getPrototypeOf(target) {
                return Reflect.getPrototypeOf(ident);
            },
            has(target, key) {
                return Reflect.has(ident, key);
            },
            isExtensible(target) {
                return Reflect.isExtensible(ident);
            },
            ownKeys(target) {
                return Reflect.ownKeys(ident);
            },
            preventExtensions(target) {
                return Reflect.preventExtensions(ident);
            },
            set(object, property, value) {
                return Reflect.set(ident, property, value);
            },
            setPrototypeOf(target, proto) {
                return Reflect.setPrototypeOf(ident, proto);
            }
        });
    }
}
...
var SubscriptMethods = define({
    instance: {
         ...
         * @param subscript
         */
        sqlSubscript: function (...args) {
            return new SubScript(this, args);
        }
    }
