    ...
    static:{
        ...
        sync:function (cb) {
            ...
            if (!this.synced && this.hasAssociations) {
                this._super().then(comb.hitch(this, function () {
                    var associations = this.__associations;
                    comb.serial(Object.keys(associations).map(function (type) {
                        return function () {
                            var types = associations[type];
                            return comb.serial(Object.keys(types).map(function (name) {
                                return function () {
                                    return types[name].model.sync();
                                };
                            }, this));
                        };
                    }, this)).then(comb.hitchIgnore(ret, "callback", this), ret);
