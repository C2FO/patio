    ...
    instance:{
        ...
        __execute:function (conn, sql, opts, cb) {
            ...
            this.__logAndExecute(sql,
                ...
                try {
                    if (type === "select") {
                        when(r ? cb(r, info) : null, hitch(ret, "callback"), hitch(ret, "errback"));
                    } else if (isFunction(cb)) {
                        when(cb(r, info), hitch(ret, "callback"), hitch(ret, "errback"));
                    } else {
                        ret.callback();
                    }
                } catch (e) {
        ...
        indexes:function (table, opts) {
            ...
            this.metadataDataset.withSql("SHOW INDEX FROM ?",
                isInstanceOf(table, sql.Identifier) ? table : sql.identifier(im(table))).forEach(
                function (r) {
