var comb = require("comb"), logging = comb.logging, Logger = logging.Logger, format = comb.string.format, QueryError = require("../errors").QueryError;
...
comb.define(null, {
    instance:{
        ...
        __logAndExecute:function (sql, args, cb) {
            ...
            args && (sql = format("%s; %j", sql, args));

            var start = new Date();
            var ret = new comb.Promise();
            if (comb.isFunction(cb)) {
                this.logInfo(format("Executing; %s", sql.trim()));
                cb().then(comb.hitch(this, function () {
                    this.logDebug(format("Duration: % 6dms; %s", new Date() - start, sql.trim()));
                    ret.callback.apply(ret, arguments);
                }), comb.hitch(this, function (err) {
                    var err = new QueryError(format("%s: %s", err.message, sql.trim()));
                    this.logError(err);
