...
var checkAndTransformName = function (name) {
    return comb.isString(name) ? applyColumnTransformMethod(name, Model.camelize === true ? "camelize" : Model.underscore === true ? "underscore" : null) : name;
};

var Model = comb.define([QueryPlugin, comb.plugins.Middleware], {
    instance:{
        /**
        ...
        _setColumnValue:function (name, val) {
            ...
            var columnValue = comb.isFunction(setterFunc) ? setterFunc.call(this, val, ignore) : val;
        ...
        _typeCastValue:function (column, value, fromDatabase) {
            ...
            if (((fromDatabase && clazz.typecastOnLoad) || (!fromDatabase && clazz.typecastOnAssignment)) && !comb.isUndefinedOrNull(this.schema) && !comb.isUndefinedOrNull((colSchema = this.schema[column]))) {
    ...
    static:{
        ...
        init:function () {
            ...
            ["addListener", "on", "once", "removeListener",
                "removeAllListeners", "setMaxListeners", "listeners", "emit"].forEach(function (name) {
                    this[name] = emitter[name].bind(emitter);
                }, this);
            if (this.__tableName) {
        ...
        sync:function (cb) {
            ...
            if (!this.synced) {
                var db = this.db, tableName = this.tableName, supers = this.__supers;
                db.schema(tableName).then(hitch(this, function (schema) {
                    if (!this.synced && schema) {
                        this._setSchema(schema);
                        if (supers && supers.length) {
                            new PromiseList(supers.map(function (sup) {
                                return sup.sync();
        ...
        load:function (vals) {
            var ret = new Promise();
            //sync our model
            this.sync().then(hitch(this, function () {
            ...
            this.sync().then(hitch(this, function () {
                if (this.useTransaction(opts)) {
                    this.db.transaction(opts, hitch(this, function () {
                        return comb.when(cb()).then(function () {
                        ...
                        if (errored) {
                            ret.errback.apply(ret, retVal);
                        } else {
                            ret.errback.apply(ret, arguments);
                        }
                    }));
                } else {
                    comb.when(cb()).then(ret);
                }
            }), ret);
            return ret;
        },
        ...
        getters:{
            ...
            dataset:function () {
                ...
                if (!ds) {
                    ...
                    ds.rowCb = hitch(this, function rowCb(vals) {
                        return this.load(vals);
                    });
                }
                return ds;
...
exports.create = function (name, supers, modelOptions) {
    ...
    model = comb.define(supers.concat(modelOptions.plugins || []).concat([AssociationPlugin]), modelOptions);
...
exports.syncModels = function (cb) {
    var ret = new comb.Promise();
    comb.serial(MODELS.entrySet.map(function (entry) {
        return function () {
            var value = entry.value;
            return comb.serial(value.entrySet.map(function (m) {
                return hitch(m.value, "sync");
            }));
        };
    })).then(hitchIgnore(ret, "callback", true), ret);
    if (isFunction(cb)) {
        ret.classic(cb);
    }
    return ret;
...
exports.getModel = function (name, db) {
    var ret = null;
    if (comb.isDefined(name)) {
        !patio && (patio = require("./index"));
        if (comb.isFunction(name)) {
            ret = name;
        } else {
            if (!db && comb.isInstanceOf(name, patio.Dataset)) {
                db = name.db;
            }
