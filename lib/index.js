...
const date = comb.date;
const SQL = require("./sql").sql;
const Promise = comb.Promise;
const PromiseList = comb.PromiseList;
const singleton = comb.singleton;
const isFunction = comb.isFunction;
const executeInOrder = comb.executeInOrder;
const argsToArray = comb.argsToArray;
const isString = comb.isString;
const patio = exports;

const LOGGER = comb.logger("patio");

const Patio = singleton([EventEmitter, Time], {
    instance: {
        /**
...
export default patio || new Patio();
patio.__Patio = Patio;
const dbAdapters = Database.ADAPTERS;
for (var i in dbAdapters) {
    patio[i] = adapters[i];
}
