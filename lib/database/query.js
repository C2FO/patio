...
var Database = define(null, {
    instance: {
            ...
            if (this.schemas[quotedName]) {
                ...
                this.schemaParseTable(tableName, opts).then(hitch(this, function (cols) {
                    ...
                    if (!cols || cols.length === 0) {
                        ret.errback(new Error("Error parsing schema, " + table + " no columns returns, table probably doesnt exist"));
                    } else {
        ...
        __transactionProxy: function (cb, conn) {
            ...
            ["transaction", "execute"].forEach(function (n) {
                ...
                this[n] = function (arg1, arg2) {
                    ...
                    promises.push(when(ret));
                    return ret.promise();
                };
            }, this);
            try {
                var transactionP = new comb.Promise();
                var cbRet = cb.apply(this, [this, transactionP.resolve.bind(transactionP)]);
                if (cbRet && isPromiseLike(cbRet, Promise)) {
                    promises.push(cbRet);
                } else {
                    promises.push(transactionP);
                }
            } catch (e) {
        ...
        __transaction: function (conn, opts, cb) {
            ...
            try {
                var connPromise = conn ? new Promise().callback(conn) : this._getConnection().addCallback(hitch(this, function (conn) {
                ...
                connPromise.then(hitch(this, function (conn) {
                    this.__beginTransaction(conn, opts).chain(hitch(this, function () {
                        this.__transactionProxy(cb, conn).chain(hitch(this, function (res) {
                            this.__commitTransaction(conn).chain(hitch(this, "__transactionComplete", promise, "callback", conn), hitch(this, "__transactionComplete", promise, "errback", conn));
                        }), hitch(this, "__rollback", promise, conn));
                    }), hitch(this, "__transactionComplete", promise, "errback", conn));
                }), hitch(this, "__transactionComplete", promise, "errback"));
            } catch (e) {
                this.logError(e);
                promise.errback(e);
            }
        ...
        __rollback: function (promise, conn, err) {
            this.__rollbackTransaction(conn, null, err).both(hitch(this, "__finishTransactionAndCheckForMore", conn)).then(hitch(this, function () {
        ...
        __beginNewTransaction: function (conn, opts) {
            return this.__logConnectionExecute(conn, this.beginTransactionSql).chain(hitch(this, "__setTransactionIsolation", conn, opts))
        },
        ...
        __setTransactionIsolation: function (conn, opts) {
            ...
            if (this.supportsTransactionIsolationLevels && !isUndefinedOrNull(level = isUndefinedOrNull(opts.isolation) ? this.transactionIsolationLevel : opts.isolation)) {
                return this.__logConnectionExecute(conn, this.__setTransactionIsolationSql(level)).then(ret);
            } else {
                ret.callback();
            }
            return ret.promise();
