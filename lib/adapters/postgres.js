...
try {
    ...
    console.warn("Could not use pg native falling back to javascript");
    pg = require("pg");
}
    ...
    instance: {
        ...
        listen: function (channel, cb, opts) {
            ...
            this._getConnection().chain(function (conn) {
                ...
                self.__logAndExecute(sql, comb.bindIgnore(conn, "query", sql)).then(function () {
        ...
        unListen: function (channel) {
            ...
            if (channel in this.__listeners && (conn = this.__listeners[channel])) {
                ...
                return this.__logAndExecute(sql, comb.bindIgnore(conn, "query", sql)).chain(function () {
        ...
        notify: function (channel, payload) {
            return this.executeDdl(this.__notifySql(channel, payload));
        },
        ...
        __notifySql: function (channel, payload) {
            return format("NOTIFY %s %s", this.__quoteSchemaTable(channel), payload ? ", " + this.literal(JSON.stringify(payload)) : "");
        },

        __listenSql: function (channel) {
            return format("LISTEN %s", this.__quoteSchemaTable(channel));
        },

        __unListenSql: function (channel) {
            return format("UNLISTEN %s", this.__quoteSchemaTable(channel));
        },
