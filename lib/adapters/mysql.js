...
var DS = comb.define(Dataset, {
    ...
    instance:{
        ...
        fullTextSql:function (cols, term, opts) {
            opts = opts || {};
            return format("MATCH %s AGAINST (%s%s)", this.literal(comb.array.toArray(cols)),
                this.literal(comb.array.toArray(term).join(" ")), opts.boolean ? " IN BOOLEAN MODE" : "");
        },
        ...
        _deleteFromSql:function (sql) {
            if (this._joinedDataset) {
                return format(" %s FROM %s", this._sourceList(this.__opts.from[0]), this._sourceList(this.__opts.from))
                    + this._selectJoinSql();
            } else {
                return this._super(arguments);
            }
        ...
        fetchRows:function (sql, block, cb) {
            return this.execute(sql, hitch(this, function (r, fields) {
                var i = -1;
                var cols;

                cols = fields.map(function (col) {
                    var fieldName = col.name;
                    var type = col.type;
                    var length = col.fieldLength;
                    return [this.outputIdentifier(fieldName), DB.convertMysqlType(type == 1 && length != 1 ? 2 : type), fieldName];
                }, this);
        ...
        __processRows:function (rows, cols, cb) {
            ...
            var ret = new comb.Promise();
            var processed = [];
            var processRow = function (rows, cols, cb, index) {
                if (index == rows.length) {
                    ret.callback(processed);
                } else {
                    try {
                        var h = {};
                        var row = rows[index];
                        ...
                        comb.when(cb(h)).then(function (val) {
                            processed.push(val);
                            process.nextTick(function () {
                                processRow(rows, cols, cb, ++index);
                            });
                        }, hitch(ret, "errback"));
                    } catch (e) {
        ...
        execute:function (sql, opts, block, cb) {
            ...
            return this._super(arguments, [sql, comb.merge({type:"select"}, opts), block], cb);
        },
    ...
    static:{
        ...
        DELETE_CLAUSE_METHODS:Dataset.clauseMethods("delete", "from where order limit"),
        ...
        SELECT_CLAUSE_METHODS:Dataset.clauseMethods("select", "distinct columns from join where group having compounds order limit lock"),
...
var DB = comb.define(Database, {
    instance:{
        ...
        createConnection:function (opts) {
            delete opts.query;
            var conn = mysql.createConnection(comb.merge({}, opts, {typeCast:false}));
        ...
        execute:function (sql, opts, cb, conn) {
            var ret = new Promise();
            if (comb.isInstanceOf(cb, Connection)) {
                conn = cb;
                cb = null;
            }
            var connP = !comb.isUndefinedOrNull(conn) ? new comb.Promise().callback(conn) : this._getConnection();
            connP.then(hitch(this, function (conn) {
        ...
        indexes:function (table, opts) {
            ...
            var ret = new comb.Promise();
            this.metadataDataset.withSql("SHOW INDEX FROM ?",
                comb.isInstanceOf(table, sql.Identifier) ? table : sql.identifier(im(table))).forEach(
                function (r) {
                    ...
                    if (name != "PRIMARY") {
                        ...
                        i.columns.push(m(r.Column_name || r.ColumnName));
                    }
                }).then(function () {
                    var r = {};
        ...
        __alterTableSql:function (table, op) {
            ...
            if (op.op == "addColumn") {
                ...
                this.schema(table).then(hitch(this, function (schema) {
                    ...
                    ret.callback(format("ALTER TABLE %s CHANGE COLUMN %s %s", this.__quoteSchemaTable(table),
                        this.__quoteIdentifier(op.name), this.__columnDefinitionSql(comb.merge(op, opts))));
                }), hitch(ret, "errback"));
                return ret;
        ...
        __beginNewTransaction:function (conn, opts) {
            ...
            this.__setTransactionIsolation(conn, opts)
                .chain(hitch(this, this.__logConnectionExecute, conn, this.beginTransactionSql),
                hitch(ret, "errback"))
        ...
        __commitTransaction:function (conn, opts) {
            ...
            if (s) {
                ...
                this.__logConnectionExecute(conn, format("XA END %s", this.literal(s)))
                    .chain(hitch(this, "__logConnectionExecute", format("XA PREPARE %s", this.literal(s))),
                    hitch(ret, "errback"))
                    .then(hitch(ret, "callback"), hitch(ret, "errback"));
                return ret;
            } else {
        ...
        __rollbackTransaction:function (conn, opts) {
            ...
            if (s) {
                s = this.literal(s);
                var ret = new comb.Promise();
                this.__logConnectionExecute(conn, "XA END " + s).chain(hitch(this, "__logConnectionExecute", conn,
                    "XA PREPARE " + s), hitch(ret, "errback")).then(hitch(this, "__logConnectionExecute", conn,
                    "XA ROLLBACK " + s), hitch(ret, "errback"));
                return ret;
            } else {
                return this._super(arguments);
            }
        ...
        schemaParseTable:function (tableName, opts) {
            var m = this.outputIdentifierFunc;
            var im = this.inputIdentifierFunc;
            return this.metadataDataset.withSql("DESCRIBE ?", sql.identifier(im(tableName))).map(hitch(this,
                function (row) {
                    var ret = {};
                    var e = row[m("Extra")];
                    ...
                    ret.type = this.schemaColumnType(ret.dbType.toString("utf8"));
    ...
    static:{
        ...
        convertMysqlType:function (type) {
            ...
            return hitch(this.FIELD_TYPES, function (type, o) {
                if (o != null) {
                    switch (type) {
                        ...
                        case this.FIELD_TYPE_YEAR:
                            convertDate(o, "stringToYear", convertDateTime);
                            break;
                        ...
                        case this.FIELD_TYPE_DECIMAL:
                            // decimal types cannot be parsed as floats because
                            // V8 Numbers have less precision than some MySQL Decimals
                            return parseFloat(o);
                            break;
                        case this.FIELD_TYPE_TINY_BLOB:
                        case this.FIELD_TYPE_MEDIUM_BLOB:
                        case this.FIELD_TYPE_LONG_BLOB:
                        case this.FIELD_TYPE_BLOB:
                            return new Buffer(o);
                            break;
                    }
                }
