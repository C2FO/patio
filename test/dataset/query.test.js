...
suite.addBatch({
    ...
    "a grouped dataset" : {
        ...
        "should raise when trying to generate a truncate statement" : function() {
            assert.throws(function() {
                dataset.truncateSql();
            });
        },
    ...
    "Dataset.literal" : {
        ...
        "should literalize TimeStamp properly"  : function(dataset) {
            ...
            assert.equal(dataset.literal(d), comb.date.format(d.date, "'yyyy-MM-dd HH:mm:ss Z'"));
        },
    ...
    "Dataset.orderMore and orderAppend" : {
        topic : new Dataset().from("test"),
        ...
        "should accept a block that yields a virtual row" : function(dataset) {
            assert.equal(dataset.order("a").orderMore(
                function(o) {
                    return o.b
                }).sql, 'SELECT * FROM test ORDER BY a, b');
            assert.equal(dataset.order("a", "b").orderMore("c", "d",
                function() {
                    return [this.e, this.f(1, 2)];
                }).sql, 'SELECT * FROM test ORDER BY a, b, c, d, e, f(1, 2)');
            assert.equal(dataset.order("a").orderAppend(
                function(o) {
                    return o.b
                }).sql, 'SELECT * FROM test ORDER BY a, b');
            assert.equal(dataset.order("a", "b").orderAppend("c", "d",
                function() {
                    return [this.e, this.f(1, 2)];
                }).sql, 'SELECT * FROM test ORDER BY a, b, c, d, e, f(1, 2)');
        }
    ...
    "Dataset.firstSourceAlias" : {
        ...
        "should be aliased as first_source" : function(ds) {
            assert.deepEqual(ds.from("t").firstSourceAlias(), new Identifier("t"));
            assert.deepEqual(ds.from(new sql.Identifier("t__a")).firstSourceAlias(), new sql.Identifier("t__a"));
            assert.equal(ds.from("s__t___a").firstSourceAlias(), "a");
            assert.equal(ds.from(sql.t.as("a")).firstSourceAlias(), "a");
        ...
        "should raise exception if table doesn't have a source" : function(ds) {
            assert.throws(hitch(ds, "firstSourceAlias"));
        }
    ...
    "Dataset.firstSourceTable" : {
        ...
        "should raise exception if table doesn't have a source" : function(ds) {
            assert.throws(hitch(ds, "firstSourceTable"));
        }
    },
    ...
    "Dataset.joinTable" : {
        ...
        "should not allow insert, update, delete, or truncate"  : function(d) {
            ...
            assert.throws(hitch(ds, "deleteSql"));
            assert.throws(hitch(ds, "truncateSql"));
        },
