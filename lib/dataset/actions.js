    ...
    instance:{
        ...
        forEach:function (block, cb) {
            ...
            if (this.__opts.graph) {
                ...
                ret = this.fetchRows(this.selectSql, function (r) {
                    var ret = new Promise();
                    try {
                        when(rowCb(r)).then(function (r) {
                            try {
                                when(block(r)).then(hitch(ret, "callback", r), ret);
                            } catch (e) {
                                ret.errback(e);
                            }
                        }, ret);
        ...
        isEmpty:function (cb) {
            ...
            this.get(1).then(hitch(this, function (res) {
                ret.callback(isUndefinedOrNull(res) || res.length === 0);
            }), ret);
            return ret.promise();
        },
        ...
        first:function (args) {
            ...
            var cb, block = isFunction(args[args.length - 1]) ? args.pop() : null;
        ...
        map:function (column, cb) {
            ...
            forEachP = this.forEach(function (r) {
                ...
                return when(item, function (mapped) {
                    a.push(mapped);
                });
            });
        ...
        execute:function (sql, opts, block) {
            ...
            return this.db.execute(sql, merge({server:this.__opts.server || "readOnly"}, opts), block);
        },
