            ...
            if (!isUndefinedOrNull(item)) {
                if (!model.isNew) {
                    if (isInstanceOf(item, this.model) && !item.isNew) {
                        ...
                        ret = new Promise();
                        model._checkTransaction(hitch(this, function () {
                            ...
                            if (loaded && reload) {
                                return this.parent._reloadAssociationsForType(this.type, this.model, model).then(hitch(ret, "callback"), hitch(ret, "errback"));
                            } else {
                                ret.callback(model);
                            }
                        }), ret);
