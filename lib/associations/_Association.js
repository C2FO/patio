...
module.exports = exports = comb.define(comb.plugins.Middleware, {
    instance:{
        ...
        _getAssociationKey:function () {
            ...
            } else if (this.supportsLeftAndRightKey && (!comb.isUndefinedOrNull((lk = options.leftKey))
                    && !comb.isUndefinedOrNull((rk = options.rightKey)))) {
                ret = [
        ...
        __setValue:function (parent, model) {
            parent.__associations[this.name] = this._fetchMethod
                    == "all" ? !comb.isArray(model) ? [model] : model : comb.isArray(model) ? model[0] : model;
