    ...
    instance : {
        ...
        typecastValueDatetime : function(value) {
            if (comb.isDate(value)) {
                ...
                var ret = comb.date.parse(value, this.DATE_TIME_PATTERN);
                if (ret == null) {
                    throw new DatabaseError(format("Invalid value for date %j", [value]));
                }
                return ret;
        ...
        typecastValueTime : function(value) {
            if (comb.isDate(value)) {
                ...
                var ret = comb.date.parse(value, this.TIME_PATTERN);
                if (ret == null) {
                    throw new DatabaseError(format("Invalid value for date %j", [value]));
                }
                return ret;
            } else if (comb.isHash(value) && !comb.isEmpty(value)) {
                return new Date(0, 0, 0, value.hour, value.minute, value.second);
            } else {
