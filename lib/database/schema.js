  ...
  instance:{
    ...
    forceDropTable:function(names) {
      ...
      if (comb.isArray(names)) {
        ...
        this.dropTable(argsToArray(arguments).filter(function(t) {
          return comb.isString(t) || comb.isInstanceOf(t, Identifier, QualifiedIdentifier);
    ...
    __typeLiteralGenericDate:function(column) {
      ...
      return column.type === Date ?
        "date" : column.onlyTime || column.type.match(/^time$/i) ?
        "time" : column.timeStamp || column.type.match(/^timestamp$/i) ?
        "timestamp" : column.dateTime || column.type.match(/^datetime/i) ?
        "datetime" : "date";
    },
