...
it.describe("patio.adapters.Postgres", function (it) {
    ...
    /*it.describe("#createTable", function (it) {
     ...
     PG_DB.forceCreateTable("test_innodb", {engine:"InnoDB"},function () {
     this.primaryKey("id");
     ...
     PG_DB.from("test2").forShare().all().classic(function (err, res) {
     ...
     return PG_DB.createTable("items",function () {
     this.name("string");
     this.value("integer");
     }).then(hitch(this, function () {
     PG_DB.sqls.length = 0;
     ...
     comb.executeInOrder(PG_DB,function (db) {
     ...
     it.beforeEach(function () {
     return comb.executeInOrder(PG_DB, function (db) {
     db.forceDropTable("items");
     db.createTable("items", function () {
     ...
     comb.executeInOrder(PG_DB, d,function (db, d) {
     d.insert();
     ...
     PG_DB.fetch("SELECT CAST('0000-00-00' AS date)").singleValue().then(function (res) {
     ret.errback("err");
     }, function () {
     PG_DB.fetch("SELECT CAST('0000-00-00 00:00:00' AS datetime)").singleValue().then(function () {
     ret.errback("err");
     }, function () {
     PG_DB.fetch("SELECT CAST('25:00:00' AS time)").singleValue().then(function () {
     ret.errback("err");
     }, function (err) {
     patio.mysql.convertInvalidDateTime = false;
     ret.callback();
     ...
     comb.executeInOrder(PG_DB, patio,
     function (db, patio) {
