...
var createData = function(){
    ...
    var classDs = DB.from("class");
    var studentDs = DB.from("student");
    var classInsertPromise = classDs.multiInsert([
        {
            semester:"FALL",
    ...
    var studentInsertPromise = studentDs.multiInsert([
        {
            firstName:"Bob",
            lastName:"Yukon",
            gpa:3.689,
...
var printResults = function(studentDs, classDs){
    //print the results
    studentDs.forEach(function(student){
            var classes = student.classes;
            console.log(format("%s %s is enrolled in %s", student.firstName, student.lastName, !classes.length ? " no classes!" : "\n\t-" + classes.map(
                function(clas){
                    return clas.name;
                }).join("\n\t-")));
...
createTables()
    .chain(createModels, disconnectError)
    .chain(createData, disconnectError)
    .then(function(){
        //Get our models
        var Class = patio.getModel("class"),
            Student = patio.getModel("student");

        var classDs = Class.order("name"),
            studentDs = Student.order("firstName", "lastName");
