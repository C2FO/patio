var comb = require("comb"),
    ...
    QueryError = require("../errors").QueryError, moose;
...
comb.define(null, {
    ...
    instance:{
        ...
        /**
         * Dataset mixin that provides functions to the {@link moose.dataset.Dataset} to
         * create SELECT, UPDATE, CREATE, and DELETE SQL statements, based off of the the
         * methods invoked in the {@link moose.dataset._Query}
         * mixin. This class should not be used directly by
         *
         * @constructs
         * @memberOf moose.dataset
         *
         * @property {String} sql Readonly property that returns a SELECT statement.
         * @property {String} deleteSql DELETE SQL query string.  See {@link moose.dataset._Actions#delete}.
         *
         *      <pre class="code">
         ...
         *              //=> "SELECT * FROM items"
         *      </pre>
         * @property {String} truncateSql Returns a TRUNCATE SQL query string.  See {@link moose.dataset._Actions#truncate}
         *
         *      <pre class="code">
         *
         ...
         *              //=> 'TRUNCATE items'
         *      </pre>
         * @property {String} exists  Returns an EXISTS clause for the dataset as a {@link moose.sql.LiteralString}.
         *
         *      <pre class="code">
         *
         *       var ds = DB.from("test");
        ...
        constructor:function () {
            ...
            !Dataset && (Dataset = moose.Dataset);
            this._super(arguments);
        },

        /**
         * Returns an INSERT SQL query string.  See {@link moose.dataset._Actions#insert}
         *
         * @example
         *
         *
         ...
         *
         * @throws {moose.errors.QueryError} if there are Different number of values and columns given to insertSql or
         ...
         *      <ul>
         *          <li>{@link moose.Dataset} : an insert with subselect will be performed.</li>
         *          <li>{@link moose.sql.LiteralString} : the literal value will be used.</li>
         *          <li>Array : the values in the array will be used as the VALUES clause.</li>
         *          <li>Object: the keys will be used as the columns, and values will be the values inserte.d</li>
         *          <li>{@link moose.sql.BooleanExpression} : the expression will be used as the values.
         *          <li>An arbitrary number of arguments : the {@link moose.dataset._Sql#literal} version of the values will be used</li>
         *      </ul>
         *
         *
         ...
         * This method is used by {@link moose.dataset._Actions#multiInsert} to format insert statements.
        ...
        /**
         *Formats an UPDATE statement using the given values.  See {@link moose.dataset._Actions#update}.
         *
         * @example
         *
         ...
         *
         * @params {*...} Variable number of values to update the table with.
         ...
         *              <li>{@link moose.sql.Expression} : the {@link moose.dataset._Sql#literal} representation of the
         *                      {@link moose.sql.Expression} will be used as the value
         *              </li>
         *              </li> Other : the {@link moose.dataset._Sql#literal} value will be used as the value</li>
         *          </ul>
         *
         ...
         *   //=> "table1"
         *
         * @param {String|moose.sql.Identifier} tableAlias the table to get an unused alias for.
         *
         ...
         *
         * @throws {moose.errors.QueryError} If an unsupported object is given.
         ...
         * @param {moose.sql.Expression} column the expression to qualify
        ...
        _qualifiedExpression:function (e, table) {
            ...
            if (comb.isString(e)) {
                ...
                return new OrderedExpression(this._qualifiedExpression(e.expression, table), e.descending, {nulls:e.nulls});
         ...
         *
         * @throws {moose.errors.QueryError} If the name is not a String {@link moose.sql.Identifier},
         * {@link moose.sql.QualifiedIdentifier} or {@link moose.sql.AliasedExpression}.
         *
         * @param {String|moose.sql.Identifier|moose.sql.QualifiedIdentifier|moose.sql.AliasedExpression} name
         ...
         * implicit aliases in strings, as well as {@link moose.sql.AliasedExpression}s.
         * Other objects are returned as is.
         *
         * @param {String|moose.sql.AliasedExpression|*} tableAlias the object to un alias
         *
         * @return {moose.sql.QualifiedIdentifier|String|*} the unaliased portion of the identifier
         */
        ...
        /**
         * Return a [@link moose.sql._Query#fromSelf} dataset if an order or limit is specified, so it works as expected
        ...
        /*
         This section is for easier adapter overrides of sql formatting. These metthods are used by moose.sql.* toString
         methods to generate sql.
         */

        /**
         * @private  For internal use by moose
         *
            ...
            if ((newOp = isOperators[op]) != null) {
                ...
                if (colArray) {
                    if (emptyValArray) {
                        ...
                        return string.format("(%s %s %s)", comb.isString(cols) ? cols : this.literal(cols), ComplexExpression.IN_OPERATORS[op], valArray ? this._arraySql(vals) : this.literal(vals));
                    }
                }
                else {
                    if (emptyValArray) {
         ...
         * quote the name with {@link moose.dataset._Sql#_quotedIdentifier}.
         */
        ...
        /**
         * @private For For internal use by moose
         *
        ...
        formatTimestamp:function (v) {
            return this.literal(moose.dateToString(v));
        },
        ...
        /**
         * @return SQL fragment for a type of object not handled by {@link moose.dataset._Sql#literal}.
         ...
         * on the dataset provided and should add that method to {@link moose.dataset.Dataset}, allowing for adapters
        ...
        _literalDate:function (date) {
            return (this.requiresSqlStandardDateTimes ? "DATE '" : "'") + moose.dateToString(date) + "'";
        },
        ...
        _literalString:function (v) {
            ...
            if (!alias) {
                return column && table ? this._literalExpression(QualifiedIdentifier.fromArgs([table, column])) : "'" + v.replace(/\\/g, "\\\\").replace(/'/g, "''") + "'"
            } else {
        ...
        _selectCompoundsSql:function () {
            ...
            if (compounds) {
                compounds.forEach(function (c) {
                    ...
                    ret += string.format(" %s%s %s", type.toUpperCase(), all ? " ALL" : "", this._subselectSql(dataset));
                }, this);
            }
            return ret;
        },
        ...
        _selectFromSql:function () {
            ...
            return from ? string.format(" FROM%s", this._sourceList(from)) : "";
        },
        ...
        /**
         * @see moose.dataset._Sql#_selectFromSql
         */
        ...
        /**
         * @see moose.dataset._Sql#_selectOrderSql
         */
        ...
        /**
         * @see moose.dataset._Sql#_selectWhereSql
         */
    ...
    static:{
        ...
        QUESTION_MARK:/\?/g,
        DELETE_CLAUSE_METHODS:["_deleteFromSql", "_deleteWhereSql"],
        INSERT_CLAUSE_METHODS:["_insertIntoSql", "_insertColumnsSql", "_insertValuesSql"],
        SELECT_CLAUSE_METHODS:["_selectWithSql", "_selectDistinctSql", "_selectColumnsSql",
            "_selectFromSql", "_selectJoinSql", "_selectWhereSql", "_selectGroupSql", "_selectHavingSql",
            "_selectCompoundsSql", "_selectOrderSql", "_selectLimitSql", "_selectLockSql"],
        UPDATE_CLAUSE_METHODS:["_updateTableSql", "_updateSetSql", "_updateWhereSql"],
        TIMESTAMP_FORMAT:"'yyyy-MM-dd HH:mm:ss Z'",
        STANDARD_TIMESTAMP_FORMAT:"TIMESTAMP 'yyyy-MM-dd HH:mm:ss Z'",
        YEAR_FORMAT:"YYYY",
