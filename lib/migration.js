...
var Migrator = comb.define(null, {
    instance:{
        ...
        getMigrationVersionFromFile:function(filename){
            return parseInt(filename.split(this._static.MIGRATION_SPLITTER)[1], 10);
        }
    },
        ...
        __getMigrator:function(directory){
            ...
            this.getFileNames(directory).then(hitch(this, function(files){
                ...
                if (l) {
                    for (var i = 0; i < l; i++) {
                        var file = files[i];
                        if (parseInt(file.split(this.MIGRATION_SPLITTER)[1], 10) > this.MINIMUM_TIMESTAMP) {
    ...
    instance:{
        ...
        run:function(){
            ...
            comb.when(this._getLatestMigrationVersion(), this._getCurrentMigrationVersion(), hitch(this, function(res){
                ...
                if (current != target) {
                    var direction = this.direction = current < target ? "up" : "down", isUp = direction === "up";
                    this._getMigrations(current, target, direction).then(hitch(this, function(migrations){
                        var runMigration = hitch(this, function(index){
                            if (index >= migrations.length) {
                                ret.callback(migrations[index - 1][1]);
                            } else {
                                var curr = migrations[index], migration = curr[0], version = curr[1];
                                var now = new Date();
                                ...
                                DB.transaction(hitch(this, function(){
                                    ...
                                    comb.when(migration[direction].apply(DB, [DB])).then(hitch(this, function(args){
                                        this._setMigrationVersion(version).then(hitch(ret, "callback"), hitch(ret, "errback"));
                                    }), hitch(ret, "errback"));
                                    return ret;
                                })).then(function(){
            ...
            db.tableExists(table).then(hitch(this, function(exists){
                if (!exists) {
                    db.createTable(table,
                        function(){
                            this.column(c, "integer", {"default":0, allowNull:false});
                        }).then(hitch(ret, "callback"), hitch(ret, "errback"));
                } else {
                    ds.columns.then(function(columns){
                        if (columns.indexOf(c) == -1) {
                            db.addColumn(table, c, "integer", {"default":0, allowNull:false})
                                .then(hitch(ret, "callback"), hitch(ret, "errback"))
                        } else {
                            ret.callback();
                        }
