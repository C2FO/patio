...
var Connection = define(null, {
    instance:{
        ...
        closeConnection:function () {
            var ret = new Promise();
            this.connection.end(ret.resolve.bind(ret));
            return ret.promise();
        },
        ...
        query:function (query) {
            ...
            try {
                ...
                this.connection.query(query, ret.resolve.bind(ret));
            } catch (e) {
                patio.logError(e);
            }
            return ret.promise();
...
var DS = define(Dataset, {
    ...
    instance:{
        ...
        execute:function (sql, opts, block, cb) {
            ...
            return this._super(arguments, [sql, merge({type:"select"}, opts), block], cb);
        },
        ...
        executeDui:function (sql, opts, block) {
            ...
            return this._super(arguments, [sql, merge({type:"dui"}, opts), block]);
        },
...
var DB = define(Database, {
    instance:{
        ...
        execute:function (sql, opts, cb, conn) {
            if (isInstanceOf(cb, Connection)) {
                conn = cb;
                cb = null;
            }
            var connP = !isUndefinedOrNull(conn) ? new Promise().callback(conn) : this._getConnection();
            return connP.chain(hitch(this, function (conn) {
                return this.__execute(conn, sql, opts, cb);
            })).promise();
        },

        __execute:function (conn, sql, opts, cb) {
            var ret = new Promise();
            this.__logAndExecute(sql,function () {
                ...
                try {
                    if (type === "select") {
                        when(r ? cb(r, info) : null).then(ret);
                    } else if (isFunction(cb)) {
                        when(cb(r, info)).then(ret);
                    } else {
                        ret.callback();
                    }
                } catch (e) {
        ...
        indexes:function (table, opts) {
            ...
            var ret = new Promise();
            this.metadataDataset.withSql("SHOW INDEX FROM ?", isInstanceOf(table, sql.Identifier) ? table : sql.identifier(im(table)))
                .forEach(function (r) {
            ...
            if (op.op === "addColumn") {
                ...
                this.schema(table).then(hitch(this, function (schema) {
                    ...
                    ret.callback(format("ALTER TABLE %s CHANGE COLUMN %s %s", this.__quoteSchemaTable(table),
                        this.__quoteIdentifier(op.name), this.__columnDefinitionSql(merge(op, opts))));
                }), ret);
            } else if (op.op === "dropIndex") {
                ret.callback(format("%s ON %s", this.__dropIndexSql(table, op), this.__quoteSchemaTable(table)));
            } else {
                ret = this._super(arguments);
            }
            return ret.promise();
        ...
        __beginNewTransaction:function (conn, opts) {
            return this.__setTransactionIsolation(conn, opts)
        ...
        __commitTransaction:function (conn, opts) {
            ...
            var s = opts.prepare;
            if (s) {
                return this.__logConnectionExecute(conn, format("XA END %s", this.literal(s)))
        ...
        __rollbackTransaction:function (conn, opts) {
            ...
            if (s) {
                ...
                return this.__logConnectionExecute(conn, "XA END " + s)
                    .chain(hitch(this, "__logConnectionExecute", conn, "XA PREPARE " + s))
                    .chain(hitch(this, "__logConnectionExecute", conn, "XA ROLLBACK " + s))
                    .promise();
            } else {
                return this._super(arguments);
            }
