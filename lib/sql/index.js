var comb = require("comb"), ExpressionError = require("../errors").ExpressionError, Dataset;
...
var virtualRow = function(name) {
    ...
    }, column ? QualifiedIdentifier.prototype : Identifier.prototype);
...
}).export(sql, "ComplexExpressionMethods");
...
}).export(sql, "NoBooleanInputMethods");
...
}).export(sql, "StringConcatenationMethods");
...
}).export(sql, "Expression");

var GenericExpression = comb.define([Expression, AliasMethods, BooleanMethods, CastMethods, ComplexExpressionMethods, InequalityMethods, NumericMethods, OrderedMethods, StringMethods, SubscriptMethods]).export(sql, "GenericExpression");
...
}).export(sql, "NegativeBooleanConstant");
...
var JoinOnClause = comb.define(JoinClause, {
    instance : {
        constructor : function(on, joinType, table, tableAlias) {
            this.__on = on;
            this.super(arguments, [joinType, table, tableAlias]);
        },
...
}).export(sql, "PlaceHolderLiteralString");
...
}).export(sql, "SQLFunction");

// Subclass of +ComplexExpression+ where the expression results
// in a numeric value in SQL.
var NumericExpression = comb.define([ComplexExpression,BitWiseMethods,NumericMethods, InequalityMethods]).export(sql, "NumericExpression");
...
}).export(sql, "QualifiedIdentifier");
...
var likeElement = function(re) {
    var ret;
    if (comb.isRexExp(re)) {
        ret = [("" + re).replace(/^\/|\/$/g, ""), true, re.ignoreCase]
    } else {
        ret = [re, false, false];
    }
    return ret;
    ...
    instance : {
        constructor : function(str) {
            ...
            STRING_METHODS.forEach(function(op) {
                this[op] = comb.hitch(this, addStringMethod(op));
            }, this);
        }
    }
});
sql.LiteralString = LiteralString;

sql.literal = function(s) {
    var args = comb.argsToArray(arguments);
    return args.length > 1 ? PlaceHolderLiteralString.fromArgs(args) : new LiteralString(s);
};

var Year = function(y) {
    this.date = Date.apply(this, [y]);
};

sql.Year = Year;

var TimeStamp = function(y, m, d, h, min, s, ms) {
    this.date = new Date(Date.apply(Date, arguments));
};
...
sql["case"] = function(hash, /*args**/opts) {
    var args = comb.argsToArray(arguments).slice(1);
