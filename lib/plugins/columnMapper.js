        ...
        sync:function () {
            ...
            if (!this.synced) {
                ret = ret.chain(function () {
                    var ds = this.dataset.naked().select(), mappedColumns = this._mappedColumns, joinTableAlias = "columnMapper", selects = [];
                        ...
                        if (isConditionSpecifier(condition)) {
                            ...
                            condition.forEach(function (cond) {
                                ...
                                if (!isInstanceOf(val, AliasedExpression) && isInstanceOf(val, Identifier)) {
                                    cond[1] = val.qualify(this.tableName);
                                }
                            }, this);
                    ...
                    var middleWare = function (next) {
                        ds.filter(this._getPrimaryKeyQuery()).qualify().one().then(function (vals) {
                            this.setValues(vals);
                        }.bind(this)).classic(next);
                    };

                    var self = this;
                    this.post("load", middleWare);
                    this.post("save", function (next, options) {
                        options = options || {};
                    ...
                    this.post("update", function (next, options) {
                        options = options || {};
